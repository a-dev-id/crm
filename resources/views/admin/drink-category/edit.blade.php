@section('title', 'Edit Drink Category')
@section('drink', 'menu-open')
@section('drink_active', 'active')
@section('drink_category_active', 'active')
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                                    ...
                                    @foreach ($drink_categories as $fc)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('drink-category.edit', [$fc->id]) }}" class="btn btn-sm btn-warning">
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <form method="POST" action="{{ route('drink-category.destroy', [$fc->id]) }}">
                                                                    @csrf
                                                                    @method('DELETE')
                ...
                <div class="col-lg-4">
                    <form method="POST" action="{{ route('drink-category.update', [$drink_category->id]) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card card-warning">
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    <label>Title</label>
                                    <input name="title" type="text" class="form-control" placeholder="Type something" value="{{ $drink_category->title }}">
                                </div>
                                <div class="form-group">
                                    <label>Description</label>
                                    <textarea name="description" class="form-control" rows="4" id="description" placeholder="Type something">
                                        {{ $drink_category->description }}
                                    </textarea>
                                </div>
                                <div class="form-group">
                                    <label>Status</label>
                                    <select name="status" class="form-control">
                                        <option value="1" @if ($drink_category == '1') selected @else @endif>Published</option>
                                        <option value="0" @if ($drink_category == '0') selected @else @endif>Draft</option>
                                    </select>
                                </div>
                            </div>
                            <div class="card-footer">
                                <button type="submit" class="btn btn-success btn-sm"><i class="fas fa-paper-plane"></i> Submit</button>
                                <a href="{{ route('drink-category.index') }}" class="btn btn-danger btn-sm"><i class="fas fa-ban"></i> Cancel</a>
                            </div>
                        </div>
                    </form>
                </div>
