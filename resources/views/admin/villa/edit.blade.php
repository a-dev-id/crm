    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                ...
                <form method="POST" action="{{ route('villa.update', [$villa->id]) }}" enctype="multipart/form-data" class="col-lg-4">
                    @csrf
                    @method('PUT')
                    <div class="card card-warning">
                        ...
                        <div class="card-body row">
                            <div class="form-group col-12">
                                ...
                                <input name="title" type="text" class="form-control" placeholder="Type something" value="{{ $villa->title }}">
                            </div>
                            <div class="form-group col-12">
                                <label>Image</label>
                                <input name="image" type="file" class="form-control" placeholder="Type something">
                                <input name="old_image" type="hidden" value="{{ $villa->image }}">
                            </div>
                            <div class="form-group col-12">
                                <label>Current Image</label><br>
                                <img src="{{ asset($villa->image) }}" style="width: 100px;" class="border p-1 rounded">
                            </div>
                            <div class="form-group col-4">
                                <label>Wide</label>
                                <input name="wide" type="text" class="form-control" placeholder="Type something" value="{{ $villa->wide }}">
                            </div>
                            <div class="form-group col-4">
                                <label>Pool Type</label>
                                <input name="pool_type" type="text" class="form-control" placeholder="Type something" value="{{ $villa->pool_type }}">
                            </div>
                            <div class="form-group col-4">
                                <label>View</label>
                                <input name="view" type="text" class="form-control" placeholder="Type something" value="{{ $villa->view }}">
                            </div>
                            <div class="form-group col-12">
                                <label>Description</label>
                                <textarea class="form-control" id="description" name="description" placeholder="Type something">{{ $villa->description }}</textarea>
                            </div>
                        </div>
                        <div class="card-footer">
                            <button type="submit" class="btn btn-success btn-sm"><i class="fas fa-paper-plane"></i> Update</button>
