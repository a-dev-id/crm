...
<x-app-layout>
    <section class="content">
        <div class="container-fluid">
            <form method="POST" action="{{ route('drink.update', [$drink->id]) }}" enctype="multipart/form-data" class="row">
                @csrf
                @method('PUT')
                <div class="col-lg-8">
                    <div class="card card-primary">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                <label>Title</label>
                                <input name="title" type="text" class="form-control" placeholder="Type something" value="{{ $drink->title }}">
                            </div>
                            <div class="form-group">
                                <label>Excerpt</label>
                ...
                <div class="col-lg-4">
                    <div class="card card-default">
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                <label>Price</label>
                                <input name="price" type="text" class="form-control" placeholder="Type something" value="{{ $drink->price }}">
                            </div>
                            <div class="form-group">
                                ...
                                <select name="drink_category_id" class="form-control">
                                    ...
                                    @foreach ($drink_categories as $fc)
                                        <option value="{{ $fc->id }}" @if ($drink->drink_category_id == $fc->id) selected @else @endif>{{ $fc->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control">
                                    <option value="1" @if ($drink->status == '1') selected @else @endif>Published</option>
                                    <option value="0" @if ($drink->status == '0') selected @else @endif>Draft</option>
                                </select>
                            </div>
                        </div>
